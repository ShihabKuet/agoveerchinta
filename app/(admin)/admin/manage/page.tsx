...
import { Eye, Edit3, Trash2, Plus, Filter } from 'lucide-react'
    ...
    <div className="px-6 py-8">
      ...
      <div className="flex gap-2 mb-5">
        {([['all', 'সব'], ['published', 'প্রকাশিত'], ['draft', 'খসড়া']] as const).map(([val, label]) => (
          <button
            ...
            className={cn(
              'px-4 py-1.5 rounded-full text-sm font-bengali-sans font-medium transition-colors',
              filter === val
                ? 'bg-ink text-white'
                : 'bg-paper-dark text-ink-muted hover:text-ink border border-[var(--color-border)]'
            )}
          >
            {label}
          </button>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1,2,3,4,5].map(i => (
            <div key={i} className="h-16 bg-paper-dark rounded-lg animate-pulse" />
          ))}
        </div>
      ) : posts.length === 0 ? (
        <div className="text-center py-16">
          <p className="font-bengali-sans text-ink-muted">কোনো লেখা নেই।</p>
        </div>
      ) : (
              ...
              {posts.map((post) => (
                <tr key={post.id} className="hover:bg-paper-dark/50 transition-colors">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    {post.category ? (
                      <span
                        className="px-2 py-0.5 rounded text-xs font-bengali-sans font-semibold text-white"
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <p className="text-xs font-bengali-sans text-ink-muted">
                      {post.published_at
                        ...
                        : formatBengaliDate(post.created_at)}
                    </p>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className={cn(
                        'px-2.5 py-1 rounded-full text-xs font-bengali-sans font-semibold transition-colors',
                        post.status === 'published'
                          ? 'bg-green-100 text-green-700 hover:bg-red-100 hover:text-red-700'
                          : 'bg-amber-100 text-amber-700 hover:bg-green-100 hover:text-green-700'
                      )}
                      title={post.status === 'published' ? 'খসড়ায় নিন' : 'প্রকাশ করুন'}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      <Link
                        href={`/${post.slug}`}
                        target="_blank"
                        className="p-1.5 rounded text-ink-muted hover:text-accent hover:bg-paper-dark transition-colors"
                        title="দেখুন"
                      >
                        <Eye size={15} />
                      </Link>
                      <button
                        ...
                        className="p-1.5 rounded text-ink-muted hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50"
                        title="মুছুন"
                      >
                        <Trash2 size={15} />
                      </button>
